 ...
 *
 */
int xml_is_comment(const char *end, int comment) {
 ...
 *
 */
struct xml_text xml_text_set(const char * v, int l) {

	struct xml_text text = { value: v, length: l, needfree: 0 };
	return text;
}
 ...
 *
 */
int xml_text_cmp(const struct xml_text * a, const struct xml_text * b) {
...
int xml_is_closing(const char *begin, struct xml_element *node) {
	...
	if (node)
		if (*++begin != '/' || memcmp(++begin, node->name.value, node->name.length))
			return 0;
	return 1;
}
 ...
 *
 */
int xml_feed(struct xml *xml, const char *buff, size_t len) {
 ...
 *
 */
int xml_node_walk(struct xml_element *node, int (*callback)(struct xml_element *, int)) {

	while (node) {
		struct xml_element *next = node->next;
		if (callback(node, 0))
			return -1;
		if (xml_node_walk(node->children, callback))
			return -1;
		if (callback(node, 1))
			return -1;
		node = next;
 ...
 *
 */
int xml_seed(struct xml *xml, char buff[], size_t max, char quote) {
	...
	int callback_node_seed(struct xml_element *node, int end) {
		...
		if (node) {
			switch (node->type) {
				case xml_attribute_node:
					if (end)
						break;
					xml_seed_LENGTH(max, len, 1);
					buff[0] = ' ';
					buff++;
					xml_seed_LENGTH(max, len, node->name.length);
					memcpy(buff, node->name.value, node->name.length);
					buff += node->name.length;
					if (node->content.value) {
						...
						xml_seed_LENGTH(max, len, node->content.length);
						memcpy(buff, node->content.value, node->content.length);
						buff += node->content.length;
						if (quote) {
							xml_seed_LENGTH(max, len, 1);
							buff[0] = quote;
							buff++;
				...
				case xml_element_node:
					...
					buff += node->name.length;
					if (!end) {
						if (xml_node_walk(node->attribute, callback_node_seed))
							return -1;
						if (node->shape == xml_shape_single) {
							xml_seed_LENGTH(max, len, 1);
							buff[0] = node->name.value[0] == '?' ? '?' : '/';
							buff++;
						}
					}
					xml_seed_LENGTH(max, len, 1);
	...
	if (xml_node_walk(xml->document.children, callback_node_seed))
		return -1;
...
void xml_dump(struct xml *xml, FILE *out) {
	...
	int callback_node_print(struct xml_element *node, int end) {
		if (end)
			return 0;
	...
	xml_node_walk(xml->document.children, callback_node_print);
}

/**
 *
 */
struct xml_element * xml_node_find(struct xml_element *node, const char *value, int length) {
 ...
 *
 */
struct xml_element * xml_node_lookup(struct xml_element *node, const char *value, int length) {
 ...
 *
 */
struct xml_element * xml_lookup(struct xml *xml, const char *value, int length) {
 ...
 *
 */
const struct xml_text * xml_entity_find(const char *entity, int length) {

	int code = *entity == '#' ? atoi(entity + 1) : 0;
	int i = 0;

	for (; i < sizeof(__xml_entity) / sizeof(*__xml_entity); i++) {
		if (code) {
 ...
 *
 */
int xml_text_merge(char *to, int amount, int length, const struct xml_text *from) {

	int l = amount;
	int i = 0;

	for (; from && l < length && i < from->length; i++) {
 ...
 *
 */
struct xml_text xml_node_text(struct xml_element *node, char *value, int length) {

	int amount = 0;

	int callback_node_text(struct xml_element *node, int end) {
		if (end)
			return 0;
		if (!node->parent || !node->parent->tag || node->parent->tag->block != xml_block_comment) {
			if (node->type == xml_text_node) {
				int len = xml_text_merge(value, amount, length, &node->content);
				amount += len;
				length -= len;
	...
	xml_node_walk(node->children, callback_node_text);
 ...
 *
 */
size_t xml_attribute_next(struct xml_element *attribute, const struct xml_text *content, int offset,
		const char *reject) {
...
struct xml_element * xml_element_init(struct xml_element *e, struct xml_element *parent, enum xml_type type,
		enum xml_shape shape, const char *name, unsigned nlen, const char *content, unsigned clen) {
 ...
 *
 */
int xml_text_cpy(char *s, int m, const struct xml_text t) {

	if (m) {
		int l = m > t.length ? t.length : m - 1;
 ...
 *
 */
char * xml_text_dup0(const char *c, int n) {
 ...
 *
 */
struct xml_text xml_text_dup(const char *s, int n) {

	struct xml_text text = { value: s && n ? xml_text_dup0(s, n) : NULL, length:s && n ? n : 0, needfree: 1 };
 ...
 *
 */
int xml_text_cmp3(const struct xml_text * text, const char *sample) {

	return strncasecmp(text->value ? : "", sample, text->length) ? : sample[text->length];
}

/**
 *
 */
int xml_text_cmp4(const struct xml_text * text, const char *sample, unsigned length) {
 ...
 *
 */
struct xml_text xml_text_escape2(const char *text, int length, char *buff, int size) {
...
struct xml_text xml_text_unescape(const struct xml_text text, char *buff, int length) {
	...
	for (; i < text.length && l < length; i++, l++) {
		switch (text.value[i]) {
			case '%':
				hex[0] = text.value[++i];
				hex[1] = text.value[++i];
 ...
 *
 */
int xml_text_cmp2(const struct xml_text a, const struct xml_text b) {
 ...
 *
 */
struct xml_text xml_text_escape(struct xml_text text, char *buff, int size) {
 ...
 *
 */
struct xml_element * xml_node_attribute(struct xml_element *node, const char *value, int length) {
 ...
 *
 */
struct xml_text xml_text_decode(const struct xml_text text, char *buff, int length) {
